...
def make_scored_job(make_job, *, score=80.0, **job_overrides) -> ScoredJob:
...
def test_sync_open_roles_appends_new_row_below_threshold_skipped(make_job):
    writer = FakeSheetsWriter()
    high = make_scored_job(make_job, score=80.0, company="Acme", title="Platform Engineer")
    low = make_scored_job(make_job, score=10.0, company="Acme", title="Sales Rep")
...
def test_sync_open_roles_rerun_does_not_duplicate_or_clobber_status(make_job):
    writer = FakeSheetsWriter()
    job = make_scored_job(
        make_job,
        score=80.0,
...
def test_sync_open_roles_dedups_same_job_from_two_sources(make_job):
    writer = FakeSheetsWriter()
...
def test_open_roles_row_maps_location_to_country_and_region(make_job):
    writer = FakeSheetsWriter()
    job = make_scored_job(
        make_job,
        company="Acme",
...
def test_open_roles_row_formats_salary_range(make_job):
    writer = FakeSheetsWriter()
    job_with_salary = make_scored_job(make_job, salary_min=90000, salary_max=130000)
